...
var simpScroller = (function() {
	...
	var _scroller = function(container, direction, params) {
		...
		var fnPosScroll = function() {
			...
			var keyScroller = container["scroll" + Key] / (sizeContainerWithScroll - sizeContainer) * (sizeContainer - sizeScroller);
			// 边界溢出的修正
			if (sizeContainer - sizeScroller - keyScroller <= 0) {
				keyScroller =parseFloat( sizeContainer) - parseFloat(sizeScroller);
			}
			// 滚动条的定位
		...
		var initPosScroll=function(wrapObj){
			sizeContainerWithScroll = wrapObj["scroll" + Size];
	...
	return function(container, options) {
		...
		if (params.verticalScroll == true) {
			scroller=_scroller(wrap, "vertical", params);	
		}
		if (params.horizontalScroll == true) {
			scroller=_scroller(wrap, "horizontal",  params);	
		}
		
		
		scroller.initPosScrollFn(wrap);
		return this;
	};
})();
