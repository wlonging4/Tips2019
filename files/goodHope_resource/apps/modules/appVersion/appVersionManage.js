'use strict';
function appVersionManageController($scope,$timeout,$location, $modal, tools,DTOptionsBuilder, DTColumnBuilder,$q){
    ...
    vm.dtOptions = DTOptionsBuilder
        .newOptions().withOption('ajax',{
            url: G.server + '/appversion/selectPageList.json',
            type: 'POST',
            data: $scope.form
        })
        .withDataProp('data')
    ...
    $scope.action = {
        ...
        add:function () {
            if($scope.form.appType==1){
                location.href='#/goodhope-appVersionAndriod?appType='+$scope.form.appType;
            }
            if($scope.form.appType==2){
                location.href='#/goodhope-appVersionIos?appType='+$scope.form.appType;
            }
        }
    };
    ...
    function fnDrawCallback(){
        ...
        $(".js_huodong_open").on("click", function(){
            ...
            $.ajax({
                type: "post",
                url: G.server + "/appversion/disable.json",
                data: data,
                dataType: "json",
                success: function(data){
                    tools.ajaxOpened(self);
        ...
        $('.editAppVersion').click(function () {
            ...
            if($scope.form.appType==1){
                location.href='#/goodhope-appVersionAndriod?appType='+$scope.form.appType+'&id='+nowId;
            }
            if($scope.form.appType==2){
                location.href='#/goodhope-appVersionIos?appType='+$scope.form.appType+'&id='+nowId;
            }
        });
