...
$.fn.multiSel = function (options){
    ...
    var defaults = {
        ...
        name: "p_status",
        data: [
            {"text":"111111","value":"1","default":true,"disabled":false}
        ],
        selectallTxt: '全选',
        selectokTxt: '确定',
    };
    ...
    return this.each(function (){
        ...
        // 保存value;
        var $ipth = $('<input type="hidden" value="" name="' + options.name + '" />');
        ...
        var $allbody = $('<div class="col-lg-8 checkbox" style="margin:0;padding: 0 7px;"><label><input type="checkbox" class="select_all"/>'+
        ...
        for(var i = 0; i < count; i++){
            var divstr='';
            if(options.data[i].disabled){
                divstr = '<div class="checkbox" style="background:#eee;"><label style="width:100%;padding: 0 7px;">'+
                    '<input class="multiSel_unit"  disabled="true" type="checkbox" value="'+options.data[i].value+'"';
            }else{
                divstr = '<div class="checkbox"><label style="width:100%;padding: 0 7px;">'+
        ...
        $all.change(function (){//点击all
            ...
            $dropList.each(function (){
                if($all.is(':checked')){
                    if(!$(this).attr('disabled')){
                        ...
                        opt_arr.push($.trim($(this).parent().parent().parent().text()));
            ...
            $.uniform.update($dropList);
            options.callbackChange && options.callbackChange($ipth);
        });
        ...
        $dropList.change(function (){//勾选选项
            var opt_arr = [];
            var opth_arr = [];
            $dropList.each(function (i){
                if ($(this).is(':checked')){
                    opt_arr.push($.trim($(this).parent().parent().parent().text()));
                    opth_arr.push($(this).val());
                }
            });
            var $dropList_selected = $content.children(".checkbox").find('input:checked');
            $ipt.val(opt_arr.join(','));
            $ipth.val(opth_arr.join(","));

            var n1 = Math.abs($dropList_selected.length);
            var n2 = Math.abs($dropList.length-$('input[disabled="true"]').length);
            (n1 === n2) ? $all.parent('span').addClass("checked"): $all.parent('span').removeClass("checked");
            //勾选后回调函数
            options.callbackChange && options.callbackChange($ipth);
        });
...
$("body").append($("<div>",{
    "id": "js_dialog_permission",
    "class": "modal fade",
    "tabindex": "1",
    "style": "z-index:10051;padding-top:20%"
}).html('<div class="modal-dialog" style="width: 580px;"><div class="modal-content"><div class="modal-header" style="border:none;"><button type="button" class="close" data-dismiss="modal" aria-label="Close">'+
    '<span aria-hidden="true">&times;</span></button><h4 class="modal-title fn-ms js_content"></h4></div></div>'));
...
;(function ($) {
    ...
    var Clipboard = function (context) {
        ...
        this.pasteByEvent = function (event) {
            ...
            if (clipboardData && clipboardData.items && clipboardData.items.length) {
                var item = clipboardData.items.length > 1 ? clipboardData.items[1] : list.head(clipboardData.items);
    ...
    var VideoDialog = function (context) {
        ...
        this.createVideoNode = function (url) {
            ...
            var baiduRegExp = /\/\/pan\.baidu\.com\//;
            var baiduMatch = url.match(baiduRegExp);

            var crediteaseRegExp = /\/\/vod\.creditease\.streamocean\.com\//;
            var crediteaseMatch = url.match(crediteaseRegExp);
            //return console.log(crediteaseMatch)
            ...
            } else if (mp4Match || oggMatch || webmMatch || (crediteaseMatch && crediteaseMatch[0].length)) {
    ...
    $.summernote = $.extend($.summernote, {
        ...
        options: {
            ...
            fontNames: [
                '宋体',"黑体",'Microsoft YaHei','Arial', 'Arial Black', 'Comic Sans MS', 'Courier New',
...
$(document).ready(function(){
     ...
     * 表格点击，行选中样式
     * **/
    $(document).on("click", "#dataTables tr", function() {
        var trs = $("#dataTables tr");
        trs.removeClass("ui_click");
        $(this).addClass("ui_click");
    });
    ...
    $(document).on("click", "#showContent", function(){
        var parentTR = $(this).parent(), trs = parentTR.siblings("tr");
