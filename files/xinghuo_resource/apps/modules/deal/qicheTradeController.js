'use strict';
function qichetradeController($scope,getSelectListFactory,getProListFactory,tools,DTOptionsBuilder, DTColumnBuilder) {
    $scope.form = {
    ...
    vm.dtColumns = [
        DTColumnBuilder.newColumn('no').withTitle('交易单号').withOption('sWidth','180px').renderWith(function(data, type, full) {
        ...
        DTColumnBuilder.newColumn('subproductname').withTitle('产品名称').withOption('sWidth','120px').renderWith(function(data, type, full) {
            return '<a href="javascript:;" class="js_trade_info ui_ellipsis" style="width: 120px;" type="goodsinfo" title="'+data+'" key="'+full.subproductid+'" action="/xinghuoproduct/productinfo.shtml">'+data+'</a>';
        }),
        ...
        DTColumnBuilder.newColumn('saleTerminal').withTitle('认购渠道').withOption('sWidth','60px').renderWith(function(data, type, full) {
            if(data == null) return "";
        ...
        DTColumnBuilder.newColumn('storecode').withTitle('店铺ID').withOption('sWidth','60px').renderWith(function(data, type, full) {
            if(!data) return "";
