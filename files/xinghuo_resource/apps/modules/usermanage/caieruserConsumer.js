...
function xinghuoConsumerController($scope,getSelectListFactory,tools,DTOptionsBuilder, DTColumnBuilder) {
    ...
    vm.dtOptions = DTOptionsBuilder
        .newOptions().withOption('ajax',{
            url: siteVar.serverUrl + '/caieruser/datatable.shtml',
            type: 'POST',
            data: function(d){
                d.orderColumn = 'createtime';
                d.orderType = 'desc';
    ...
    vm.dtColumns = [
        DTColumnBuilder.newColumn('id').withTitle('ID').withOption('sWidth','60px').renderWith(function(data,type,full){
        ...
        DTColumnBuilder.newColumn('documentno').withTitle('证件号码').withOption('sWidth','100px').notSortable(),
        DTColumnBuilder.newColumn('mobile').withTitle('手机号').withOption('sWidth','90px').notSortable(),
        DTColumnBuilder.newColumn('isleaderuser').withTitle('旗舰店注册用户').withOption('sWidth','120px').notSortable(),
        ...
        DTColumnBuilder.newColumn('id').withTitle('查看交易').withOption('sWidth','100px').renderWith(function(data,type,full){
